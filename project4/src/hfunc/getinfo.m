...
%getuserinfo( path ) returns [ userfeatures ratings ]
    ...
    for n=1:size(X,1)
        for m=1:size(X,2)
            if X(n,m) ~= 0
                userfeatures(counter, :) = [user_id(n) movie_id(m) user_age(n) ...
                    user_sex(n)  user_zipcode(n) user_occupation(n) ...
                    movie_genrecoded(m,:) timestamps(n,m)/10e6];
                counter = counter + 1;
            end
        end
    end
